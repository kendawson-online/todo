...
function showList(cobj = null) {
    ...
    if (getData) {
        ...
        if (Array.isArray(myItems) && myItems.length) {
            ...
            for (i = 0; i < num; i++) {
                html += `
      <li class="${myItems[i].status === 'completed' ? 'checked ' : ''}list-group-item" data-id="${myItems[i].id}">
        <div class="container-fluid m-0 p-0">
          <div class="d-flex flex-row align-items-center">
            <div class="p-2 flex-grow-1">${myItems[i].text}</div>
            ...
            <img src="assets/drag.svg" class="drag-handle" title="Drag to change list order">
          </div>
...
function loadSampleData() {
    let item = [
        ...
        {
            "id": 5,
            "text": "Click the 'Edit' button to modify the text of an item",
            "created": "2024-02-01T17:45:54.485Z",
            "updated": "2024-02-01T17:45:54.485Z",
            "status": "active"
        },
